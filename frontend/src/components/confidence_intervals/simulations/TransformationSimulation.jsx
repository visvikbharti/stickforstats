...
const TransformationSimulation = ({ projectId }) => {
  ...
  const handleRunSimulation = () => {
    ...
    const simulationParams = {
      action: 'start_transformation_simulation',
      params: {
    ...
    // Add distribution parameters based on parameter type
    switch (parameterType) {
      case 'VARIANCE':
      case 'STD_DEV':
        simulationParams.params.dist_params = {
          mean: 0, // Typically 0 for simulating variances
          std: distributionParams.std
        };
        break;
      case 'MEAN_POS':
        simulationParams.params.dist_params = {
          mean: Math.max(1, distributionParams.mean), // Ensure positive mean
          std: distributionParams.std
        };
        break;
      case 'PROPORTION':
