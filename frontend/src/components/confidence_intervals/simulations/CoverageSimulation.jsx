...
const CoverageSimulation = ({ projectId }) => {
  ...
  // WebSocket setup for real-time simulation updates
  useEffect(() => {
    // Only setup WebSocket if we have a projectId
    if (!projectId) return;

    // Create WebSocket connection
    const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
    const wsUrl = `${protocol}//${window.location.host}/ws/confidence_intervals/simulation/${projectId}/`;
  ...
  const handleRunSimulation = () => {
    if (!projectId) {
      setError('Please select a project before running a simulation.');
      return;
    }
    ...
    // Create simulation parameters based on selected interval type
    const simulationParams = {
      action: 'start_coverage_simulation',
      params: {
    ...
    // Add distribution parameters based on distribution type
    switch (distributionType) {
      case 'NORMAL':
        simulationParams.params.dist_params = {
    ...
    // Add additional options specific to interval type
    if (intervalType.includes('PROPORTION')) {
      simulationParams.params.method = additionalOptions.method;
    } else if (intervalType === 'VARIANCE' || intervalType === 'STD_DEV') {
      simulationParams.params.equivariance = additionalOptions.equivariance;
    } else if (intervalType === 'DIFFERENCE_MEANS') {
      simulationParams.params.equal_variances = additionalOptions.equal_variances;
    }

    // Send the simulation request via WebSocket
    if (webSocketRef.current && webSocketRef.current.readyState === WebSocket.OPEN) {
      webSocketRef.current.send(JSON.stringify(simulationParams));
      setSimulationStatus('running');
    } else {
      setError('WebSocket connection not available. Please refresh the page and try again.');
      setLoading(false);
    }
  };
  ...
  const renderResults = () => {
    ...
    const coverageChartData = [
      { name: 'Actual', value: result.coverage_rate * 100 },
      { name: 'Nominal', value: confidenceLevel * 100 }
    ];

    // Prepare data for the interval widths histogram (if available)
    const widthsData = result.width_histogram ? 
      result.width_histogram.map((bin, index) => ({
        name: bin.bin_center.toFixed(2),
        value: bin.count
      })) : [];

    // Calculate coverage difference for display
    const coverageDiff = (result.coverage_rate * 100) - (confidenceLevel * 100);
        ...
        <Grid container spacing={3}>
            ...
            <Paper sx={{ p: 2, height: '100%' }}>
              ...
              <ResponsiveContainer width="100%" height={300}>
                ...
                >
                  ...
                  <YAxis 
                    domain={[
                      Math.min(confidenceLevel * 100 * 0.9, result.coverage_rate * 100 * 0.9),
                      Math.max(confidenceLevel * 100 * 1.05, result.coverage_rate * 100 * 1.05)
                    ]}
              ...
              <Typography variant="body2" sx={{ mt: 2 }}>
                <strong>Actual Coverage:</strong> {(result.coverage_rate * 100).toFixed(1)}%
              </Typography>
          ...
          <Grid item xs={12} md={6}>
            <Paper sx={{ p: 2, height: '100%' }}>
              ...
              <Typography variant="body2" sx={{ mt: 2 }}>
                <strong>Average Interval Width:</strong> {result.mean_interval_width.toFixed(4)}
              </Typography>
              <Typography variant="body2">
                <strong>Median Interval Width:</strong> {result.median_interval_width.toFixed(4)}
              </Typography>
            </Paper>
          </Grid>
        </Grid>
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} md={6}>
              <Box sx={{ mb: 2 }}>
                ...
                <Typography variant="body2">
                  <strong>Average Width:</strong> {result.mean_interval_width.toFixed(4)}
                </Typography>
                <Typography variant="body2">
                  <strong>Number of Intervals Containing True Parameter:</strong> {result.intervals_containing_true_param} 
