...
const BootstrapSimulation = ({ projectId }) => {
  ...
  const handleRunSimulation = () => {
    ...
    switch (distributionType) {
      ...
      case 'LOGNORMAL':
        simulationParams.params.dist_params = {
          location: distributionParams.location,
          scale: distributionParams.std
        };
        break;
      case 'GAMMA':
        simulationParams.params.dist_params = {
      ...
      case 'UNIFORM':
        simulationParams.params.dist_params = {
          low: distributionParams.mean - distributionParams.std * Math.sqrt(3),
          high: distributionParams.mean + distributionParams.std * Math.sqrt(3)
        };
        break;
      case 'SKEWED':
        simulationParams.params.dist_params = {
          location: distributionParams.location,
          scale: distributionParams.scale,
          skew: distributionParams.skew
        };
        break;
      case 'MIXTURE':
        simulationParams.params.dist_params = {
          p1: distributionParams.p1,
          p2: 1 - distributionParams.p1,
          mean1: distributionParams.mean1,
          mean2: distributionParams.mean2,
          std1: distributionParams.std1,
          std2: distributionParams.std2
        };
        break;
      case 'USER':
        // Parse user data and send it directly
        const parsedData = parseUserData();
        simulationParams.params.dist_params = {
          user_data: parsedData.data
        };
        break;
      default:
        simulationParams.params.dist_params = {
