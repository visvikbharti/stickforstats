...
# Service imports
from ..analytics.statistical.types import AnalysisResult
...
class ReportGeneratorService:
    ...
    def generate_report_from_analyses(self, 
                                   ...
                                   include_raw_data: bool = False) -> Tuple[Dict[str, Any], BytesIO]:
        """
        Generate a report from a list of analyses.
        
        Args:
            user_id: The ID of the user generating the report
        ...
        Returns:
            A tuple of (Report metadata dict, report binary data)
        """
        ...
        report_data['config'] = report_config
        
        # Create report ID
        report_id = str(uuid.uuid4())
        report_data['id'] = report_id
        
        # Add analyses to report data
        for analysis in analyses:
            # Extract analysis data for report
            analysis_data = {
                'id': str(getattr(analysis, 'id', uuid.uuid4())),
                'name': getattr(analysis, 'name', 'Analysis'),
                'type': getattr(analysis, 'analysis_type', 'Unknown'),
                'parameters': getattr(analysis, 'parameters', {}),
                'results': analysis.results if hasattr(analysis, 'results') else {},
                'visualizations': []
            }
            
            # Include visualizations if requested
            if include_visualizations and hasattr(analysis, 'visualizations'):
                for viz in analysis.visualizations:
                    analysis_data['visualizations'].append({
                        'id': str(getattr(viz, 'id', uuid.uuid4())),
                        'title': getattr(viz, 'title', 'Visualization'),
                        'type': getattr(viz, 'visualization_type', 'plot'),
                        'description': getattr(viz, 'description', ''),
                        'figure_data': getattr(viz, 'figure_data', {})
                    })
        ...
        report_metadata = {
            ...
            'analyses': [analysis_data['id'] for analysis_data in report_data['analyses']]
        }
    ...
    def get_user_reports(self, user_id: str, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        """
        try:
            # Get list of report files in reports directory
            reports = []
            for report_file in self.reports_dir.glob(f"report_*.pdf"):
                # Extract report ID from filename
                report_id = report_file.stem.replace('report_', '')
                
                # Try to load report metadata
                metadata_file = self.reports_dir / f"metadata_{report_id}.json"
    ...
    def generate_pdf_report(self, report_data: Dict[str, Any]) -> BytesIO:
        ...
        # Add report metadata
        story.append(Paragraph(f"Generated for: User {report_data['user_id']}", self.custom_styles['Normal']))
        story.append(Paragraph(f"Date: {report_data['date']}", self.custom_styles['Normal']))
        story.append(Paragraph(f"Report ID: {report_data['id']}", self.custom_styles['Normal']))
        ...
        for i, analysis in enumerate(report_data['analyses']):
            ...
            if analysis['visualizations'] and PLOTLY_AVAILABLE:
    ...
    def _add_ttest_section(self, story: List, results: Dict[str, Any]):
        ...
        if 'p_value' in results:
            if results['p_value'] < 0.05:
                ...
                summary_data.append(["Significance", "Not significant (p ≥ 0.05)"])
    ...
    def _add_anova_section(self, story: List, results: Dict[str, Any]):
        ...
        if 'effect_size' in results:
            story.append(Paragraph(f"Effect Size (η²): {results['effect_size']:.4f}", self.custom_styles['Normal']))
            story.append(Spacer(1, 0.1*inch))
    ...
    def _add_chi_squared_section(self, story: List, results: Dict[str, Any]):
        ...
        if 'p_value' in results:
            if results['p_value'] < 0.05:
                ...
                stats_data.append(["Significance", "Not significant (p ≥ 0.05)"])
    ...
    def _figure_to_image(self, figure_data: Dict[str, Any]) -> Optional[BytesIO]:
        ...
        if not PLOTLY_AVAILABLE:
            logger.warning("Plotly is not available, skipping figure conversion")
            return None
    ...
    def generate_html_report(self, report_data: Dict[str, Any]) -> BytesIO:
        ...
        <body>
            ...
            <p>Generated for: User {report_data['user_id']}</p>
    ...
    def export_to_pdf(self, 
        ...
        Args:
            title: Report title
            content: List of report sections
            output_path: Optional path to save the PDF file
            
        Returns:
            BytesIO containing the PDF data
        """
        # Build report data structure expected by generate_pdf_report
        report_data = {
            'title': title,
            'description': '',
            'user_id': 'system',
        ...
        # Convert ReportSection objects to the expected format
        for section in content:
            ...
            for plot in section.plots:
                analysis['visualizations'].append({
                    ...
                    'type': plot.get('type', 'plot'),
                    'description': plot.get('description', ''),
                    'figure_data': plot.get('data', {})
                })
...
def get_report_generator_service() -> ReportGeneratorService:
    """Get the singleton instance of the report generator service."""
    return ReportGeneratorService.get_instance()