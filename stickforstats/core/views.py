...
from .models import Dataset, Analysis, Report, Visualization, Workflow
...
class DashboardView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['report_count'] = Report.objects.filter(user=user).count()
        ...
        context['recent_reports'] = Report.objects.filter(user=user).order_by('-created_at')[:5]
